...
export const metadata: Metadata = {
  title: "RISE 升昇电气｜新能源配套与低压配电解决方案",
  description: "浙江升昇电气面向新能源、工业制造与工程建设项目，提供光伏配套、低压成套、储能控制与电能质量治理产品支持。",
  ...
  return <PageFrame>
    <main>
      <section className="hero hero-image" id="top">
        <img className="hero-photo" src="/assets/factory/factory-demo.png" alt="RISE 电气制造与配电场景示意图" fetchPriority="high" />
      ...
      <section className="products section-shell"><SectionIntro mark="02" eyebrow="产品中心" title={<>面向项目，<br /><span>组织一整套电气能力。</span></>} lead="从新能源侧的连接与保护，到用电侧的配电与治理，RISE 以产品族的方式帮助客户快速找到合适的起点。" /><div className="product-grid">{products.map((product) => <article className={`product-card ${product.tone}`} key={product.slug}><div className="product-image visual-image"><img src={product.asset} alt={`${product.title}产品示意图`} loading="lazy" /></div><div className="product-number">{product.index}</div><div className="product-copy"><h3>{product.title}</h3><p>{product.desc}</p><Link className="card-link" href={`/products/${product.slug}`}>查看产品详情 <span aria-hidden="true">→</span></Link></div></article>)}</div><Link className="section-more" href="/products">查看全部产品系列 <span aria-hidden="true">→</span></Link></section>

      <section className="solutions"><div className="section-shell"><SectionIntro dark mark="03" eyebrow="解决方案" title={<>不同场景，<br /><em>同一份稳定。</em></>} lead="无论是新能源项目的快速落地，还是工厂配电的长期运行，我们从真实工况出发，让产品和现场更匹配。" /><div className="scenario-grid">{solutions.map((item) => <Link className="scenario" href={`/solutions/${item.slug}`} key={item.slug}><div className="scenario-image"><img src={item.image} alt={`${item.title}应用场景示意图`} loading="lazy" /></div><span>{item.index}</span><div><h3>{item.title}</h3><p>{item.desc}</p></div><b aria-hidden="true">→</b></Link>)}</div></div></section>

      <section className="cases-preview section-shell"><SectionIntro mark="04" eyebrow="工程案例" title={<>让交付能力，<br /><span>被具体项目看见。</span></>} lead="案例内容正在收集与整理中。上线前将替换为真实项目现场、技术难点和交付结果。" /><div className="case-grid">{cases.map((item) => <Link className="case-card" href={`/cases/${item.slug}`} key={item.slug}><div className="case-image visual-image"><img src={item.image} alt={`${item.title}案例示意图`} loading="lazy" /></div><div className="case-copy"><span>{item.tag}</span><h3>{item.title}</h3><p>{item.desc}</p><b aria-hidden="true">→</b></div></Link>)}</div><Link className="section-more" href="/cases">查看案例栏目 <span aria-hidden="true">→</span></Link></section>

      <section className="capability section-shell"><div className="capability-visual visual-image"><img src="/assets/factory/factory-demo.png" alt="RISE 生产实力示意图，真实车间照片待补" loading="lazy" /><div className="image-stamp">MANUFACTURING<br /><strong>IN PROGRESS</strong></div></div><div className="capability-copy"><div className="section-mark">/ 05</div><p className="eyebrow">制造与协同</p><h2>让复杂的项目，<br /><span>拥有清晰的下一步。</span></h2><p className="capability-lede">从第一次沟通到出厂交付，我们把信息、图纸、配置和检查节点放在同一条链路里，让每一次合作都更可复盘、更可复制。</p><div className="workflow-list">{[["01", "需求沟通", "从应用场景、技术参数、数量与交期开始，快速明确项目边界。"], ["02", "方案确认", "围绕系统图、柜体布局、元器件与接口进行技术评审。"], ["03", "生产检验", "按确认后的图纸与配置执行装配、接线、测试和出厂资料整理。"], ["04", "交付协同", "提供包装、铭牌、文件与发运信息，协助项目进入安装阶段。"]].map(([number, title, desc]) => <div className="workflow" key={number}><span>{number}</span><div><h3>{title}</h3><p>{desc}</p></div></div>)}</div></div></section>
